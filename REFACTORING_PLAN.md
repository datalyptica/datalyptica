# 🏗️ Data Lakehouse Repository Refactoring Plan

**Date:** December 1, 2025  
**Scope:** Architectural cleanup to enforce Data Lakehouse standards  
...
| ----------------------------- | ------------- | ----- |
| Directory Structure           | ⚠️ Partial    | 60%   |
| Docker Standards              | ⚠️ Needs Work | 70%   |
| Single Responsibility         | ✅ Good       | 85%   |
| Configuration Externalization | ⚠️ Partial    | 50%   |
...
- `./configs/monitoring/` - ✓ Monitoring configs already in correct location

#### 📦 Files to Move

**From `docker/config/` → `./configs/`:**

- [ ] `docker/config/haproxy/haproxy.cfg` → `./configs/haproxy/haproxy.cfg`
- [ ] `docker/config/minio/minio.conf` → `./configs/minio/minio.conf`
- [ ] `docker/config/postgresql/pg_hba.conf` → `./configs/postgresql/pg_hba.conf`
- [ ] `docker/config/postgresql/postgresql.conf` → `./configs/postgresql/postgresql.conf`
- [ ] `docker/config/spark/spark-defaults.conf` → `./configs/spark/spark-defaults.conf`
- [ ] `docker/config/spark/spark-env.sh` → `./configs/spark/spark-env.sh`
- [ ] `docker/config/trino/catalog/iceberg.properties` → `./configs/trino/catalog/iceberg.properties`
- [ ] `docker/config/trino/config.properties` → `./configs/trino/config.properties`
- [ ] `docker/config/trino/log.properties` → `./configs/trino/log.properties`
- [ ] `docker/config/trino/node.properties` → `./configs/trino/node.properties`
- [ ] `docker/config/nessie/application.properties` (if exists) → `./configs/nessie/application.properties`
- [ ] `docker/config/great-expectations/example_expectations.py` → `./configs/great-expectations/example_expectations.py`

**From `docker/services/*/config/` → `./configs/`:**

- [ ] `docker/services/dbt/config/profiles.yml` → `./configs/dbt/profiles.yml`
- [ ] `docker/services/patroni/config/patroni.yml` → `./configs/patroni/patroni.yml`

**Configuration Templates to Create:**

- [ ] `./configs/airflow/airflow.cfg.template` → Extract from Dockerfile ENV vars
- [ ] `./configs/jupyterhub/jupyterhub_config.py.template` → Extract from service
- [ ] `./configs/superset/superset_config.py.template` → Extract from service
- [ ] `./configs/mlflow/mlflow.conf` → Document MLflow configuration
- [ ] `./configs/kafka/server.properties` → Document Kafka config
- [ ] `./configs/flink/flink-conf.yaml` → Document Flink config
...
- [ ] `docker/services/airflow/Dockerfile` → `./deploy/docker/airflow/Dockerfile`
- [ ] `docker/services/clickhouse/Dockerfile` → `./deploy/docker/clickhouse/Dockerfile`
- [ ] `docker/services/dbt/Dockerfile` → `./deploy/docker/dbt/Dockerfile`
- [ ] `docker/services/flink/Dockerfile` → `./deploy/docker/flink/Dockerfile`
- [ ] `docker/services/great-expectations/Dockerfile` → `./deploy/docker/great-expectations/Dockerfile`
- [ ] `docker/services/jupyterhub/Dockerfile` → `./deploy/docker/jupyterhub/Dockerfile`
- [ ] `docker/services/jupyterlab-notebook/Dockerfile` → `./deploy/docker/jupyterlab-notebook/Dockerfile`
- [ ] `docker/services/kafka/Dockerfile` → `./deploy/docker/kafka/Dockerfile`
- [ ] `docker/services/kafka-connect/Dockerfile` → `./deploy/docker/kafka-connect/Dockerfile`
- [ ] `docker/services/minio/Dockerfile` → `./deploy/docker/minio/Dockerfile`
- [ ] `docker/services/mlflow/Dockerfile` → `./deploy/docker/mlflow/Dockerfile`
- [ ] `docker/services/nessie/Dockerfile` → `./deploy/docker/nessie/Dockerfile`
- [ ] `docker/services/patroni/Dockerfile` → `./deploy/docker/patroni/Dockerfile`
- [ ] `docker/services/postgresql/Dockerfile` → `./deploy/docker/postgresql/Dockerfile`
- [ ] `docker/services/schema-registry/Dockerfile` → `./deploy/docker/schema-registry/Dockerfile`
- [ ] `docker/services/spark/Dockerfile` → `./deploy/docker/spark/Dockerfile`
- [ ] `docker/services/superset/Dockerfile` → `./deploy/docker/superset/Dockerfile`
- [ ] `docker/services/trino/Dockerfile` → `./deploy/docker/trino/Dockerfile`
- [ ] `docker/services/zookeeper/Dockerfile` → `./deploy/docker/zookeeper/Dockerfile`

#### Entrypoint Scripts to Move (with Dockerfiles):

- [ ] `docker/services/*/scripts/*.sh` → `./deploy/docker/*/scripts/`

**Total: 19 Dockerfiles + ~25 scripts = 44 files**

---

## 🐋 **PHASE 2: Docker Standardization**

### 2.1 Dockerfiles Using Latest/Generic Tags ⚠️

**Issue:** No `:latest` tags found (✅ GOOD)  
...
## ⚙️ **PHASE 3: Configuration Externalization**
...
### 3.2 Secrets to Environment Variables ✅

**Current Status:** ✅ GOOD - Already using Docker secrets and .env  
...
## 🔨 **PHASE 4: Single Responsibility Principle**

### 4.1 Multi-Purpose Scripts to Split

#### ✅ GOOD - Most scripts are already atomic:
...
**Impact:** 1 script → 4 atomic scripts

---

### 4.2 Build Scripts Structure ✅
...
**No other tutorial code found** ✅

---

### 5.3 Non-Stack Services ✅

**Audit Result:** All services are within allowed stack:

- ✅ Iceberg, Nessie, MinIO (Storage/Catalog)
- ✅ Trino, Flink, Spark (Compute)
- ✅ Kafka, Debezium (Ingestion) - Debezium via kafka-connect
- ✅ Airflow (Orchestration)
- ✅ Prometheus, Grafana, Loki (Observability)
- ✅ Additional: PostgreSQL, Keycloak, ClickHouse, dbt (supporting services)

**No non-stack services to remove** ✅
...
**Status:** ✅ Low priority (minimal Python code)
...
- **Script Splits:** 1 script → 4 scripts
