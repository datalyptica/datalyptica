...
func (g *Generator) getServiceEnvironmentVariables(name string, service *ServiceConfig, config *ComposeConfiguration) map[string]string {
	...
	case "nessie":
		...
		envVars["POSTGRES_HOST"] = "${POSTGRES_HOST}"
		envVars["POSTGRES_DB"] = "${POSTGRES_DB}"
		...
		envVars["QUARKUS_DATASOURCE_POSTGRESQL_JDBC_URL"] = "jdbc:postgresql://${POSTGRES_HOST}:5432/${POSTGRES_DB}"
		...
		envVars["QUARKUS_DATASOURCE_JDBC_URL"] = "jdbc:postgresql://${POSTGRES_HOST}:5432/${POSTGRES_DB}"
...
func (g *Generator) BuildEnvContent(config *ComposeConfiguration) string {
	...
	for name, service := range config.Services {
		if service.Enabled {
			if name == "nessie" || name == "postgresql" {
				buf.WriteString("POSTGRES_HOST=postgresql\n")
			}
...
func (g *Generator) getServiceEnvFileVariables(name string, service *ServiceConfig, config *ComposeConfiguration) map[string]string {
	...
	case "postgresql":
		...
		envVars["POSTGRES_HOST"] = "postgresql"
	...
	case "prometheus":
		...
		envVars["PROMETHEUS_RETENTION_SIZE"] = "10GB"
		envVars["PROMETHEUS_SCRAPE_INTERVAL"] = "15s"
		envVars["PROMETHEUS_EVALUATION_INTERVAL"] = "15s"

	case "grafana":
		envVars["GRAFANA_PORT"] = "3000"
		envVars["GF_SECURITY_ADMIN_USER"] = "admin"
		envVars["GF_SECURITY_ADMIN_PASSWORD"] = g.getPassword("grafana", config.Environment)
		envVars["GF_INSTALL_PLUGINS"] = "grafana-piechart-panel,grafana-clock-panel"
		envVars["GF_SECURITY_ALLOW_EMBEDDING"] = "true"
		envVars["GF_AUTH_ANONYMOUS_ENABLED"] = "false"
		envVars["GF_ANALYTICS_REPORTING_ENABLED"] = "false"
		envVars["GF_ANALYTICS_CHECK_FOR_UPDATES"] = "false"
	}
...
// ServiceListResult represents available services
...
// GenerateFiles generates Docker Compose and environment files
func (g *Generator) GenerateFiles(projectName, networkName, environment string, services map[string]interface{}, globalConfig map[string]string) (*GenerationResult, error) {
	g.logger.LogInfo("Generating compose files", map[string]interface{}{
	...
	composeContent := g.BuildComposeContent(config)

	// Generate env content
	envContent := g.BuildEnvContent(config)

	// Write files
	composeFile := filepath.Join(g.outputDir, "docker-compose.yml")
	envFile := filepath.Join(g.outputDir, ".env")
	...
	g.logger.LogInfo("Files generated successfully", map[string]interface{}{
...
func (g *Generator) ValidateConfiguration(projectName string, services map[string]interface{}) (*ValidationResult, error) {
	g.logger.LogInfo("Validating configuration", map[string]interface{}{
		"project_name": projectName,
		"services":     len(services),
	})
	...
	if len(services) == 0 {
		errors = append(errors, "At least one service must be configured")
	}
	...
	result := &ValidationResult{
		...
		Details: map[string]interface{}{
			...
			"validation_run": time.Now().Format(time.RFC3339),
		},
	}

	g.logger.LogInfo("Configuration validation completed", map[string]interface{}{
		"valid":       valid,
...
func (g *Generator) GetAvailableServices() *ServiceListResult {
	g.logger.LogInfo("Getting available services", nil)
	...
	services := []ServiceInfo{
		...
		},
		{
			Name:         "nessie",
			DisplayName:  "Nessie",
			Description:  "Data catalog with Git-like versioning",
			Category:     "infrastructure",
			Required:     true,
			Dependencies: []string{"postgresql"},
			Ports: []PortInfo{
				{Internal: 19120, External: 19120, Protocol: "tcp"},
			},
		},
		{
			Name:         "trino",
			...
			Category:     "compute",
			Required:     false,
			Dependencies: []string{"nessie", "minio"},
			Ports: []PortInfo{
		...
		},
		{
			...
			Dependencies: []string{"nessie", "minio"},
			Ports: []PortInfo{
				{Internal: 4040, External: 4040, Protocol: "tcp"},
			},
		},
		{
			Name:        "prometheus",
			...
			Required:    false,
			Ports: []PortInfo{
				{Internal: 9090, External: 9090, Protocol: "tcp"},
			},
		},
		{
			Name:         "grafana",
	...
	categories := []string{"infrastructure", "compute", "monitoring"}
...
// GetServiceTemplate returns configuration template for a service
func (g *Generator) GetServiceTemplate(serviceName string) (map[string]interface{}, error) {
	g.logger.LogInfo("Getting service template", map[string]interface{}{
		"service": serviceName,
	})
	...
	templates := map[string]map[string]interface{}{
		...
		"nessie": {
			...
			"environment": map[string]string{
				...
				"QUARKUS_DATASOURCE_URL":    "jdbc:postgresql://postgresql:5432/nessie",
			},
			"ports":      []string{"19120:19120"},
			"depends_on": []string{"postgresql"},
		},
	}

	template, exists := templates[serviceName]
	if !exists {
		return nil, fmt.Errorf("service template not found: %s", serviceName)
	}

	return template, nil
}

// PreviewConfiguration generates preview of configuration without writing files
func (g *Generator) PreviewConfiguration(projectName, networkName, environment string, services map[string]interface{}, globalConfig map[string]string) (*GenerationResult, error) {
	g.logger.LogInfo("Previewing configuration", map[string]interface{}{
		"project_name": projectName,
		"environment":  environment,
		"services":     len(services),
	})
	...
	g.logger.LogInfo("Configuration preview generated", map[string]interface{}{
		"compose_size": len(composeContent),
		"env_size":     len(envContent),
	})
