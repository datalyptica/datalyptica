...
wait_for_postgresql() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if pg_isready -h postgresql -p 5432 -U "$QUARKUS_DATASOURCE_USERNAME" -d "$POSTGRES_DB" >/dev/null 2>&1; then
...
start_nessie() {
    ...
    echo "Database: jdbc:postgresql://postgresql:5432/$POSTGRES_DB"
