...
The ShuDL platform implements **PostgreSQL High Availability (HA)** using:
...
| `shudl`        | Application user                | `shudl_password`                 |
...
DATABASE_URL=postgresql://shudl:password@localhost:5432/shudl

# NEW (via PgBouncer)
DATABASE_URL=postgresql://shudl:password@localhost:6432/shudl
```
...
| **Database Size**             | `SELECT pg_size_pretty(pg_database_size('shudl'));`                                        | Monitor growth             |
...
The ShuDL platform includes **Prometheus** for metrics collection. PostgreSQL metrics are exported via:
...
docker exec -it docker-postgresql-patroni-1 pg_dump -U postgres -d shudl -F c -f /tmp/shudl_backup.dump

# Copy backup out
docker cp docker-postgresql-patroni-1:/tmp/shudl_backup.dump /backup/
...
**Maintained By**: ShuDL DevOps Team
