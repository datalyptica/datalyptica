...
This guide documents the SSL/TLS encryption setup for the Datalyptica platform. All services are configured to use HTTPS for secure communication.
...
#### **Linux:**
```bash
sudo cp secrets/certificates/ca/ca-cert.pem /usr/local/share/ca-certificates/datalyptica-ca.crt
